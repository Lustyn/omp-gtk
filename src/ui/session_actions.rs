...
impl BranchPickerView {
    pub(crate) fn new(on_select: impl Fn(String) + 'static) -> Self {
        ...
        let heading = gtk::Label::new(Some("Branch from a message"));
        heading.set_xalign(0.0);
        heading.add_css_class("branch-picker-heading");
        let subtitle = gtk::Label::new(Some(
            "Choose a user message. omp will start a new conversation from that point.",
        ));
        ...
        let close = icons::icon_button(icons::Icon::X, "Close branch picker");
    ...
    pub(crate) fn show_loading(&self) {
        self.show_state(
            "Loading branch points…",
            "Fetching user messages from the current omp conversation.",
            true,
        );
    }
    ...
    pub(crate) fn show_branching(&self) {
        self.show_state(
            "Creating branch…",
            "Waiting for omp to open the new conversation.",
            true,
        );
    }
    ...
    pub(crate) fn show_error(&self, message: &str) {
        self.show_state(
            ...
            if message.trim().is_empty() {
                "omp rejected the branch request."
            } else {
                message
            },
            false,
    ...
    pub(crate) fn set_candidates(&self, candidates: Vec<BranchMessage>) {
        ...
        if candidates.is_empty() {
            self.show_state(
                ...
                "Send a message in this conversation before creating a branch.",
                false,
            );
            return;
        }
        ...
        for (position, candidate) in candidates.iter().enumerate() {
            let row = branch_row(candidate, position + 1);
...
impl HandoffView {
    pub(crate) fn new() -> Self {
        ...
        root.set_size_request(560, 360);
        root.set_margin_top(24);
        root.set_margin_bottom(20);
        root.set_margin_start(24);
        root.set_margin_end(24);
        root.set_accessible_role(gtk::AccessibleRole::Dialog);
        root.update_property(&[gtk::accessible::Property::Label("Hand off conversation")]);
        root.add_css_class("handoff-dialog");

        let heading = gtk::Label::new(Some("Hand off this conversation?"));
        heading.set_xalign(0.0);
        heading.add_css_class("handoff-heading");
        let detail = gtk::Label::new(Some(
            "omp will summarize the current context and start a new conversation with that summary. The current conversation remains available in Recent.",
        ));
        ...
        let instructions_label = gtk::Label::new(Some("Focus instructions (optional)"));
...
fn branch_row(candidate: &BranchMessage, position: usize) -> (gtk::ListBoxRow, gtk::Button) {
    ...
    let accessible_label =
        format!("Branch from message {position}: {accessible_preview}");
    ...
    let marker = gtk::Label::new(Some(&position.to_string()));
    ...
    let label = gtk::Label::new(Some(&format!("Message {position}")));
    ...
    text.append(&preview);
    content.append(&text);
    content.append(&icons::icon(icons::Icon::GitBranchPlus, 17));
...
mod tests {
    ...
    fn selection_uses_entry_id_when_text_and_order_are_ambiguous() {
        ...
        assert_eq!(selection_for_entry(&candidates, "Repeat this request"), None);
    }
